...
allinertia <- c(seq(from = 0.0, to = 1.0, by = 0.1))
final <- list(length(allinertia))
placer <- 0
for(inertia in allinertia){
  placer <- placer + 1
...
chris <- do.call("rbind", final)
write.csv(chris, file = "data/modeloutput-varyinertia.csv", row.names = F)
